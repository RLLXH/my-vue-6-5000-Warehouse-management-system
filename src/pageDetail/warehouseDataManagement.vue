  ...
  <div>
    ...
    <el-table :data="dataList" style="width: 100%" border>
      ...
      <el-table-column label="是否禁用" prop="prohibit">
        <template slot-scope="scope">
          <div>
            <span>{{scope.row.prohibit?'是':'否'}}</span>
          </div>
        </template>
      </el-table-column>
    ...
    <el-dialog title="修改库房" :visible.sync="dialogVisibleDetail" width="30%">
       <el-form
        label-position="right"
        label-width="100px"
        :inline="true"
      ...
      >
        ...
        <el-form-item label="库房编码:" prop="storageRoomCode">
          <el-input v-model="updateData.storageRoomCode"></el-input>
        </el-form-item>
        <el-form-item label="库房地址:" prop="address">
...
export default {
  ...
  methods: {
   ...
   this.$refs[formName].validate(valid => {
        if (valid) {
          axios.put(storageRoomUpdate+'?id='+this.updateData.id,this.updateData).then(data=>{
